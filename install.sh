...
# Figure out which non-root user should own generated artifacts (admin.txt, app.py, etc.)
INSTALL_USER="${SUDO_USER:-purplelab}"
...
if ! curl -fsS --max-time 10 https://github.com >/dev/null; then
  echo -e "${RED}✖ No Internet connectivity (HTTPS). Please ensure you have an active Internet connection.${NC}"
  exit 1
else
...
required_ram_kb=8283750 # ~7.9 GiB in kB
if [ "${total_ram}" -lt "${required_ram_kb}" ]; then
...
ansible-playbook -i inventory/local/hosts playbook.yml --tags "common,webserver,database" --extra-vars "purplelab_user=${INSTALL_USER}"

# Exécuter le playbook OpenSearch séparément
echo -e "${YELLOW}Installing OpenSearch...${NC}"
ansible-playbook -i inventory/opensearch/hosts/hosts inventory/opensearch/opensearch_only.yml --extra-vars "purplelab_user=${INSTALL_USER}"

# Exécuter la partie VirtualBox
echo -e "${YELLOW}Installing VirtualBox and configuring VMs...${NC}"
ansible-playbook -i inventory/local/hosts playbook.yml --tags "virtualbox" --extra-vars "purplelab_user=${INSTALL_USER}"
...
if [ ! -f "/home/${INSTALL_USER}/admin.txt" ] || grep -q "^Password: $" "/home/${INSTALL_USER}/admin.txt"; then
  ...
  cat > /home/${INSTALL_USER}/admin.txt << EOL
...
echo -e "${GREEN}*                                           *${NC}"
echo -e "${GREEN}*********************************************${NC}"
echo -e "\nCheck the file /home/${INSTALL_USER}/admin.txt for all credentials."
echo -e "Access PurpleLab via: http://$(hostname -I | awk '{print $1}')"

if [ -n "$SANDBOX_IP" ]; then
    echo -e "Access Sandbox VM via RDP: $SANDBOX_IP"
    echo -e "Sandbox VM IP: $SANDBOX_IP" >> /home/${INSTALL_USER}/admin.txt
else
    ...
    echo -e "Sandbox VM access instructions saved to /home/${INSTALL_USER}/admin.txt"
fi 
