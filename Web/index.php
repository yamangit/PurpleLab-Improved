...
$customPayloadsCount = 0;

$activeUsersResult = pg_query($conn, "SELECT COUNT(*) AS count FROM users");
if ($activeUsersResult && $row = pg_fetch_assoc($activeUsersResult)) {
    $activeUsersCount = $row['count'];
}

$mitreCountResult = pg_query($conn, "SELECT COUNT(DISTINCT technique_id) AS count FROM atomic_tests");
if ($mitreCountResult && $row = pg_fetch_assoc($mitreCountResult)) {
    $mitreTechniqueCount = $row['count'];
}

$payloadCountResult = pg_query($conn, "SELECT COUNT(*) AS count FROM custom_payloads");
if ($payloadCountResult && $row = pg_fetch_assoc($payloadCountResult)) {
    $customPayloadsCount = $row['count'];
}

pg_free_result($result);
pg_free_result($chartDataResult);
if ($activeUsersResult) {
    pg_free_result($activeUsersResult);
}
if ($mitreCountResult) {
    pg_free_result($mitreCountResult);
}
if ($payloadCountResult) {
    pg_free_result($payloadCountResult);
}
pg_close($conn);
...
<div class="index-card">
    ...
    <div class="kpi-number" id="activeUsersCount"><?php echo $activeUsersCount; ?></div>
...
<div class="index-card">
    ...
    <div class="kpi-number" id="mitre-technique-count"><?php echo $mitreTechniqueCount; ?></div>
...
<div class="index-card">
    ...
    <div class="kpi-number" id="custom-payloads-count"><?php echo $customPayloadsCount; ?></div>
